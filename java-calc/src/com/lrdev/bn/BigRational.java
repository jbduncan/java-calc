...
//	implements Comparable<BigRational>
{
	...
	/** serialVersionUID */
  private static final long serialVersionUID = -149497000679803404L;
  ...
  /** Numerator.
	...
	*/
	public BigRational(String s, int radix) {
		...
		if (slash != -1) {
			...
			normalizeFrom((new BigRational(sn, radix)).divide(
				new BigRational(sq, radix)));
			return;
		}
	...
	*/
	public BigRational(BigInteger unscaledValue, int scale, int radix) {
		...
		normalizeFrom(
			(negt ? bigIntegerMultiply(unscaledValue, scaleValue) : unscaledValue),
	...
	private static BigRational fromIeee754(long[] value0, int fractionSize, int exponentSize) {
		...
		if (rawExponent == exponentMask) {
			...
			throw new NumberFormatException(rawFraction == 0 && fraction0.isZero() ?
				(rawSign == 0 ? "Infinity" : "-Infinity") : "NaN");
		}
	...
	*/
	...
	private String toStringDot(int precision, int radix, boolean exponentFormat) {
		...
		BigRational scaleValue = new BigRational(
			bigIntegerPower(bigIntegerValueOf(radix),
	...
	*/
	...
	private int logarithm(int base) {
		...
		final BigRational lbase = (base == 10 ? LOGARITHM_TEN_GUESS
	...
	*/
	...
	public BigRational multiply(BigRational that) {
		...
		return new BigRational(
			bigIntegerMultiply(m_n, that.m_n),
			bigIntegerMultiply(m_q, that.m_q));
	}
	...
	*/
	private BigRational remainderOrModulusOfPositive(BigRational that) {
		...
		return new BigRational(
			bigIntegerMultiply(m_n, that.m_q).remainder(
				bigIntegerMultiply(m_q, that.m_n)),
			bigIntegerMultiply(m_q, that.m_q));
	}
	...
	*/
	...
	/** Run tests.
	* <P>
	* Implementation is commented out at distribution time;
	* run at development and integration time.
	* <P>
	* Asserts conditions;
	* use '-ea' java startup option to enable asserts.
	*/
...
/*
		...
		// long/long ctor
		assert ((new BigRational(21, 35)).equals(new BigRational(3, 5)));
		assert ((new BigRational(-21, 35)).equals(new BigRational(-3, 5)));
		assert ((new BigRational(-21, 35)).equals(new BigRational(3, -5)));
		assert ((new BigRational(21, -35)).equals(new BigRational(-3, 5)));
		assert ((new BigRational(-21, -35)).equals(new BigRational(3, 5)));
		...
		// long ctor
		assert ((new BigRational(1)).equals(new BigRational(1)));
		assert ((new BigRational(0)).equals(new BigRational(0)));
		assert ((new BigRational(2)).equals(new BigRational(2)));
		assert ((new BigRational(-1)).equals(new BigRational(-1)));
		...
		assert ((new BigRational("11")).toString().equals("11"));
		assert ((new BigRational("-11")).toString().equals("-11"));
		assert ((new BigRational("+11")).toString().equals("11"));

		assert ((new BigRational("21/35")).toString().equals("3/5"));
		assert ((new BigRational("-21/35")).toString().equals("-3/5"));
		assert ((new BigRational("21/-35")).toString().equals("-3/5"));
		// special, but defined
		assert ((new BigRational("-21/-35")).toString().equals("3/5"));
		...
		// 1/x
		assert ((new BigRational("/3")).toString().equals("1/3"));
		assert ((new BigRational("-/3")).toString().equals("-1/3"));
		assert ((new BigRational("/-3")).toString().equals("-1/3"));
		// x/1
		assert ((new BigRational("3/")).toString().equals("3"));
		assert ((new BigRational("-3/")).toString().equals("-3"));
		assert ((new BigRational("3/-")).toString().equals("-3"));
		assert ((new BigRational("-3/-")).toString().equals("3"));
		// special, but defined
		assert ((new BigRational("/")).toString().equals("1"));
		assert ((new BigRational("-/")).toString().equals("-1"));
		assert ((new BigRational("/-")).toString().equals("-1"));
		...
		// "BigRational normalization"
		assert ((new BigRational("3.4")).toString().equals("17/5"));
		assert ((new BigRational("-3.4")).toString().equals("-17/5"));
		assert ((new BigRational("+3.4")).toString().equals("17/5"));

		// "BigRational missing leading/trailing zero"
		assert ((new BigRational("5.")).toString().equals("5"));
		assert ((new BigRational("-5.")).toString().equals("-5"));
		assert ((new BigRational(".7")).toString().equals("7/10"));
		assert ((new BigRational("-.7")).toString().equals("-7/10"));
		// special, but defined
		...
		// "BigRational radix"
		assert ((new BigRational("f/37", 0x10)).toString().equals("3/11"));
		assert ((new BigRational("f.37", 0x10)).toString().equals("3895/256"));
		assert ((new BigRational("-dcba.efgh", 23)).toString().equals("-46112938320/279841"));
		assert ((new BigRational("1011101011010110", 2)).toString(0x10).equals("bad6"));
		...
		assert ((new BigRational("2.5/3")).toString().equals("5/6"));
		assert ((new BigRational("2/3.5")).toString().equals("4/7"));
		assert ((new BigRational("2.5/3.5")).toString().equals("5/7"));
		assert ((new BigRational("-2.5/3")).toString().equals("-5/6"));
		assert ((new BigRational("-2/3.5")).toString().equals("-4/7"));
		assert ((new BigRational("-2.5/3.5")).toString().equals("-5/7"));
		assert ((new BigRational("2.5/-3")).toString().equals("-5/6"));
		...
		// slash nesting
		try {
			assert ((new BigRational("2/3/5")).toString().equals("2/15"));
			assert (false);
		} catch (NumberFormatException e) {
			// empty
		}
		// assert ((new BigRational("2/3/5/7")).toString().equals("2/105"));
		// similar tests with sign, dot, etc, left out for the time being

		// representations with exponent
		assert ((new BigRational("1E2")).toString().equals("100"));
		assert ((new BigRational("1E1")).toString().equals("10"));
		assert ((new BigRational("1E0")).toString().equals("1"));
		assert ((new BigRational("1E-1")).toString().equals("1/10"));
		assert ((new BigRational("1E-2")).toString().equals("1/100"));
		assert ((new BigRational("-65.4E-3")).toString().equals("-327/5000"));
		assert ((new BigRational("7.89629601826806E13")).toStringDot(4).equals("78962960182680.6000"));

		// special representations with exponent

		assert ((new BigRational("1E+1")).toString().equals("10"));
		assert ((new BigRational("1E+0")).toString().equals("1"));
		assert ((new BigRational("1E-0")).toString().equals("1"));
		assert ((new BigRational("1E")).toString().equals("1"));
		assert ((new BigRational("1E+")).toString().equals("1"));
		assert ((new BigRational("1E-")).toString().equals("1"));

		assert ((new BigRational("77E+1")).toString().equals("770"));
		assert ((new BigRational("77E+0")).toString().equals("77"));
		assert ((new BigRational("77E-0")).toString().equals("77"));
		assert ((new BigRational("77E")).toString().equals("77"));
		assert ((new BigRational("77E+")).toString().equals("77"));
		assert ((new BigRational("77E-")).toString().equals("77"));

		assert ((new BigRational("E")).toString().equals("1"));
		assert ((new BigRational("E+1")).toString().equals("10"));
		assert ((new BigRational("E+0")).toString().equals("1"));
		assert ((new BigRational("E-0")).toString().equals("1"));
		assert ((new BigRational("E+")).toString().equals("1"));
		assert ((new BigRational("E-")).toString().equals("1"));
		assert ((new BigRational("+E")).toString().equals("1"));
		...
		assert ((new BigRational("12.34/56.78")).toString().equals("617/2839"));
		assert ((new BigRational("12.34E1/56.78")).toString().equals("6170/2839"));
		assert ((new BigRational("12.34/56.78E1")).toString().equals("617/28390"));
		assert ((new BigRational("12.34E1/56.78E1")).toString().equals("617/2839"));
		assert ((new BigRational("12.34E-1/56.78")).toString().equals("617/28390"));
		assert ((new BigRational("12.34/56.78E-1")).toString().equals("6170/2839"));
		assert ((new BigRational("12.34E-1/56.78E-1")).toString().equals("617/2839"));
		assert ((new BigRational("-12.34/56.78")).toString().equals("-617/2839"));
		assert ((new BigRational("-12.34E1/56.78")).toString().equals("-6170/2839"));
		assert ((new BigRational("-12.34/56.78E1")).toString().equals("-617/28390"));
		assert ((new BigRational("-12.34E1/56.78E1")).toString().equals("-617/2839"));
		...
		// exponent nesting
		try {
			assert ((new BigRational("1E2E3")).equals((new BigRational(10)).power(2000)));
			assert (false);
		} catch (NumberFormatException e) {
			// empty
		}
		// assert ((new BigRational("1E2E3")).equals(new BigRational("1E2000")));
		// assert ((new BigRational("-1E2E3")).equals(new BigRational("-1E2000")));
		// assert ((new BigRational("1E-2E3")).equals(new BigRational("1E-2000")));
		// assert ((new BigRational("-1E-2E3")).equals(new BigRational("-1E-2000")));
		// // assert ((new BigRational("1E2E-3")).equals(new BigRational("1E0.002")));
		...
		assert (BigRational.ONE == BigRational.ONE);
		assert (BigRational.ONE.equals(BigRational.ONE));
		try {
			assert (BigRational.ONE.clone() != BigRational.ONE);
			assert (BigRational.ONE.clone().equals(BigRational.ONE));
			assert (BigRational.ONE.equals(BigRational.ONE.clone()));
		} catch (CloneNotSupportedException e) {
			assert (false);
		}
		assert (new BigRational(BigRational.ONE) != BigRational.ONE);
		assert ((new BigRational(BigRational.ONE)).equals(BigRational.ONE));
		assert (BigRational.ONE.equals(new BigRational(BigRational.ONE)));

		// scaling

		// zero scale
		// "BigRational normalization"
		assert ((new BigRational(123456, 0, BigRational.DEFAULT_RADIX)).toString().equals("123456"));
		assert ((new BigRational(123456, 1, BigRational.DEFAULT_RADIX)).toString().equals("61728/5"));
		assert ((new BigRational(123456, 2, BigRational.DEFAULT_RADIX)).toString().equals("30864/25"));
		assert ((new BigRational(123456, 5, BigRational.DEFAULT_RADIX)).toString().equals("3858/3125"));
		// <1
		assert ((new BigRational(123456, 6, BigRational.DEFAULT_RADIX)).toString().equals("1929/15625"));
		// negative scale
		assert ((new BigRational(123456, -1, BigRational.DEFAULT_RADIX)).toString().equals("1234560"));
		assert ((new BigRational(123456, -2, BigRational.DEFAULT_RADIX)).toString().equals("12345600"));

		// "BigRational constants"
		assert (BigRational.ZERO.toString().equals("0"));
		assert (BigRational.ONE.toString().equals("1"));
		assert (BigRational.MINUS_ONE.toString().equals("-1"));

		// predicates (isPositive, isNegative, isZero, isOne, isMinusOne, isInteger)

		assert (!BigRational.ZERO.isPositive());
		assert (!(new BigRational(0)).isPositive());
		assert (BigRational.ONE.isPositive());
		assert ((new BigRational(1)).isPositive());
		assert (!BigRational.MINUS_ONE.isPositive());
		assert (!(new BigRational(-1)).isPositive());
		assert ((new BigRational(77)).isPositive());
		assert (!(new BigRational(-77)).isPositive());
		assert ((new BigRational(3, 5)).isPositive());
		assert (!(new BigRational(-3, 5)).isPositive());

		assert (!BigRational.ZERO.isNegative());
		assert (!(new BigRational(0)).isNegative());
		assert (!BigRational.ONE.isNegative());
		assert (!(new BigRational(1)).isNegative());
		assert (BigRational.MINUS_ONE.isNegative());
		assert ((new BigRational(-1)).isNegative());
		assert (!(new BigRational(77)).isNegative());
		assert ((new BigRational(-77)).isNegative());
		assert (!(new BigRational(3, 5)).isNegative());
		assert ((new BigRational(-3, 5)).isNegative());

		assert (BigRational.ZERO.isZero());
		assert ((new BigRational(0)).isZero());
		assert (!BigRational.ONE.isZero());
		assert (!(new BigRational(1)).isZero());
		assert (!BigRational.MINUS_ONE.isZero());
		assert (!(new BigRational(-1)).isZero());
		assert (!(new BigRational(77)).isZero());
		assert (!(new BigRational(-77)).isZero());
		assert (!(new BigRational(3, 5)).isZero());
		assert (!(new BigRational(-3, 5)).isZero());

		assert (!BigRational.ZERO.isOne());
		assert (!(new BigRational(0)).isOne());
		assert (BigRational.ONE.isOne());
		assert ((new BigRational(1)).isOne());
		assert (!BigRational.MINUS_ONE.isOne());
		assert (!(new BigRational(-1)).isOne());
		assert (!(new BigRational(77)).isOne());
		assert (!(new BigRational(-77)).isOne());
		assert (!(new BigRational(3, 5)).isOne());
		assert (!(new BigRational(-3, 5)).isOne());

		assert (!BigRational.ZERO.isMinusOne());
		assert (!(new BigRational(0)).isMinusOne());
		assert (!BigRational.ONE.isMinusOne());
		assert (!(new BigRational(1)).isMinusOne());
		assert (BigRational.MINUS_ONE.isMinusOne());
		assert ((new BigRational(-1)).isMinusOne());
		assert (!(new BigRational(77)).isMinusOne());
		assert (!(new BigRational(-77)).isMinusOne());
		assert (!(new BigRational(3, 5)).isMinusOne());
		assert (!(new BigRational(-3, 5)).isMinusOne());

		assert (BigRational.ZERO.isInteger());
		assert ((new BigRational(0)).isInteger());
		assert (BigRational.ONE.isInteger());
		assert ((new BigRational(1)).isInteger());
		assert (BigRational.MINUS_ONE.isInteger());
		assert ((new BigRational(-1)).isInteger());
		assert ((new BigRational(77)).isInteger());
		assert ((new BigRational(-77)).isInteger());
		assert (!(new BigRational(3, 5)).isInteger());
		assert (!(new BigRational(-3, 5)).isInteger());

		// "BigRational string dot"

		assert ((new BigRational("1234.5678")).toStringDot(4).equals("1234.5678"));
		assert ((new BigRational("1234.5678")).toStringDot(5).equals("1234.56780"));
		assert ((new BigRational("1234.5678")).toStringDot(6).equals("1234.567800"));
		assert ((new BigRational("1234.5678")).toStringDot(3).equals("1234.568"));
		assert ((new BigRational("1234.5678")).toStringDot(2).equals("1234.57"));
		assert ((new BigRational("1234.5678")).toStringDot(1).equals("1234.6"));
		assert ((new BigRational("1234.5678")).toStringDot(0).equals("1235"));
		assert ((new BigRational("1234.5678")).toStringDot(-1).equals("1230"));
		assert ((new BigRational("1234.5678")).toStringDot(-2).equals("1200"));
		assert ((new BigRational("1234.5678")).toStringDot(-3).equals("1000"));
		assert ((new BigRational("1234.5678")).toStringDot(-4).equals("0"));
		assert ((new BigRational("1234.5678")).toStringDot(-5).equals("0"));
		assert ((new BigRational("1234.5678")).toStringDot(-6).equals("0"));

		assert ((new BigRational("8765.4321")).toStringDot(-2).equals("8800"));
		assert ((new BigRational("0.0148")).toStringDot(6).equals("0.014800"));
		...
		assert ((new BigRational("0.0")).toStringDot(0).equals("0"));
		assert ((new BigRational("0.0")).toStringDot(-1).equals("0"));
		assert ((new BigRational("0.0")).toStringDot(-2).equals("0"));
		assert ((new BigRational("0.0")).toStringDot(-3).equals("0"));
		assert ((new BigRational("0.0")).toStringDot(-4).equals("0"));

		// "BigRational string dot radix"
		assert ((new BigRational("1234.5678", 20)).toStringDot(3, 20).equals("1234.567"));
		assert ((new BigRational("abcd.5b7g", 20)).toStringDot(-2, 20).equals("ac00"));

		// logarithm; rounded floor
		// commented out since method is not public
		// assert ((new BigRational("1")).logarithm(10) == 0);
		// assert ((new BigRational("1.0001")).logarithm(10) == 0);
		// assert ((new BigRational("1.1")).logarithm(10) == 0);
		// assert ((new BigRational("2")).logarithm(10) == 0);
		// assert ((new BigRational("5")).logarithm(10) == 0);
		// assert ((new BigRational("9.999")).logarithm(10) == 0);
		// assert ((new BigRational("10")).logarithm(10) == 1);
		// assert ((new BigRational("10.0001")).logarithm(10) == 1);
		// assert ((new BigRational("99")).logarithm(10) == 1);
		// assert ((new BigRational("100")).logarithm(10) == 2);
		// assert ((new BigRational("999")).logarithm(10) == 2);
		// assert ((new BigRational("1000")).logarithm(10) == 3);
		// assert ((new BigRational(".5")).logarithm(10) == -1);
		// assert ((new BigRational(".2")).logarithm(10) == -1);
		// assert ((new BigRational(".10001")).logarithm(10) == -1);
		// assert ((new BigRational(".1")).logarithm(10) == -2);
		// assert ((new BigRational(".0999")).logarithm(10) == -2);
		// assert ((new BigRational(".010001")).logarithm(10) == -2);
		// assert ((new BigRational(".01")).logarithm(10) == -3);
		// assert ((new BigRational(".00999")).logarithm(10) == -3);
		// assert ((new BigRational(".001")).logarithm(10) == -4);

		// assert ((new BigRational("1")).logarithm(5) == 0);
		// assert ((new BigRational("1.0001")).logarithm(5) == 0);
		// assert ((new BigRational("1.1")).logarithm(5) == 0);
		// assert ((new BigRational("2")).logarithm(5) == 0);
		// assert ((new BigRational("4")).logarithm(5) == 0);
		// assert ((new BigRational("4.999")).logarithm(5) == 0);
		// assert ((new BigRational("5")).logarithm(5) == 1);
		// assert ((new BigRational("5.0001")).logarithm(5) == 1);
		// assert ((new BigRational("24")).logarithm(5) == 1);
		// assert ((new BigRational("25")).logarithm(5) == 2);
		// assert ((new BigRational("124")).logarithm(5) == 2);
		// assert ((new BigRational("125")).logarithm(5) == 3);
		// assert ((new BigRational(".5")).logarithm(5) == -1);
		// assert ((new BigRational(".3")).logarithm(5) == -1);
		// assert ((new BigRational(".20001")).logarithm(5) == -1);
		// assert ((new BigRational(".2")).logarithm(5) == -2);
		// assert ((new BigRational(".1999")).logarithm(5) == -2);
		// assert ((new BigRational(".040001")).logarithm(5) == -2);
		// assert ((new BigRational(".04")).logarithm(5) == -3);
		// assert ((new BigRational(".03999")).logarithm(5) == -3);
		// assert ((new BigRational(".008")).logarithm(5) == -4);

		// assert (BigRational.valueOf("2E0").logarithm(10) == 0);
		// assert (BigRational.valueOf("2E1").logarithm(10) == 1);
		// assert (BigRational.valueOf("2E2").logarithm(10) == 2);
		// assert (BigRational.valueOf("2E20").logarithm(10) == 20);
		// assert (BigRational.valueOf("2E200").logarithm(10) == 200);
		...
		assert ((new BigRational("1234.5678")).toStringDotRelative(8).equals("1234.5678"));
		assert ((new BigRational("1234.5678")).toStringDotRelative(9).equals("1234.56780"));
		assert ((new BigRational("1234.5678")).toStringDotRelative(10).equals("1234.567800"));
		assert ((new BigRational("1234.5678")).toStringDotRelative(7).equals("1234.568"));
		assert ((new BigRational("1234.5678")).toStringDotRelative(6).equals("1234.57"));
		assert ((new BigRational("1234.5678")).toStringDotRelative(5).equals("1234.6"));
		assert ((new BigRational("1234.5678")).toStringDotRelative(4).equals("1235"));
		assert ((new BigRational("1234.5678")).toStringDotRelative(3).equals("1230"));
		assert ((new BigRational("1234.5678")).toStringDotRelative(2).equals("1200"));
		assert ((new BigRational("1234.5678")).toStringDotRelative(1).equals("1000"));
		assert ((new BigRational("1234.5678")).toStringDotRelative(0).equals("0"));
		assert ((new BigRational("1234.5678")).toStringDotRelative(-1).equals("0"));
		assert ((new BigRational("1234.5678")).toStringDotRelative(-2).equals("0"));
		...
		assert ((new BigRational("0.00012345678")).toStringDotRelative(10).equals("0.0001234567800"));
		...
		assert ((new BigRational("0.00012345678")).toStringDotRelative(1).equals("0.0001"));
		assert ((new BigRational("0.00012345678")).toStringDotRelative(0).equals("0"));
		assert ((new BigRational("0.00012345678")).toStringDotRelative(-1).equals("0"));
		assert ((new BigRational("0.00012345678")).toStringDotRelative(-2).equals("0"));
		...
		assert ((new BigRational("8765.4321")).toStringDotRelative(8).equals("8765.4321"));
		assert ((new BigRational("8765.4321")).toStringDotRelative(9).equals("8765.43210"));
		assert ((new BigRational("8765.4321")).toStringDotRelative(10).equals("8765.432100"));
		assert ((new BigRational("8765.4321")).toStringDotRelative(7).equals("8765.432"));
		assert ((new BigRational("8765.4321")).toStringDotRelative(6).equals("8765.43"));
		assert ((new BigRational("8765.4321")).toStringDotRelative(5).equals("8765.4"));
		assert ((new BigRational("8765.4321")).toStringDotRelative(4).equals("8765"));
		assert ((new BigRational("8765.4321")).toStringDotRelative(3).equals("8770"));
		assert ((new BigRational("8765.4321")).toStringDotRelative(2).equals("8800"));
		assert ((new BigRational("8765.4321")).toStringDotRelative(1).equals("9000"));
		assert ((new BigRational("8765.4321")).toStringDotRelative(0).equals("0"));
		assert ((new BigRational("8765.4321")).toStringDotRelative(-1).equals("0"));
		assert ((new BigRational("8765.4321")).toStringDotRelative(-2).equals("0"));
		...
		assert ((new BigRational(1499)).toStringDotRelative(1).equals("1000"));
		assert ((new BigRational(1500)).toStringDotRelative(1).equals("2000"));
		assert ((new BigRational(1501)).toStringDotRelative(1).equals("2000"));
		assert ((new BigRational(1001)).toStringDotRelative(1).equals("1000"));
		assert ((new BigRational(1000)).toStringDotRelative(1).equals("1000"));
		assert ((new BigRational(999)).toStringDotRelative(1).equals("1000"));
		assert ((new BigRational(951)).toStringDotRelative(1).equals("1000"));
		assert ((new BigRational(950)).toStringDotRelative(1).equals("1000"));
		assert ((new BigRational(949)).toStringDotRelative(1).equals("900"));

		// check inverted radix power cases too
		assert ((new BigRational(1, 1)).toStringDotRelative(1).equals("1"));
		assert ((new BigRational(1, 2)).toStringDotRelative(1).equals("0.5"));
		assert ((new BigRational(1, 3)).toStringDotRelative(1).equals("0.3"));
		assert ((new BigRational(1, 5)).toStringDotRelative(1).equals("0.2"));
		assert ((new BigRational(1, 10)).toStringDotRelative(1).equals("0.1"));
		assert ((new BigRational(1, 20)).toStringDotRelative(1).equals("0.05"));
		assert ((new BigRational(1, 30)).toStringDotRelative(1).equals("0.03"));
		assert ((new BigRational(1, 50)).toStringDotRelative(1).equals("0.02"));
		assert ((new BigRational(1, 100)).toStringDotRelative(1).equals("0.01"));
		assert ((new BigRational(1, 200)).toStringDotRelative(1).equals("0.005"));
		assert ((new BigRational(1, 300)).toStringDotRelative(1).equals("0.003"));
		assert ((new BigRational(1, 500)).toStringDotRelative(1).equals("0.002"));
		assert ((new BigRational(1, 1000)).toStringDotRelative(1).equals("0.001"));
		assert ((new BigRational(1, 4)).toStringDotRelative(1).equals("0.3"));
		assert ((new BigRational(1, 11)).toStringDotRelative(1).equals("0.09"));
		assert ((new BigRational(1, 12)).toStringDotRelative(1).equals("0.08"));
		assert ((new BigRational(1, 31)).toStringDotRelative(1).equals("0.03"));
		assert ((new BigRational(1, 99)).toStringDotRelative(1).equals("0.01"));
		assert ((new BigRational("0.0100001")).toStringDotRelative(1).equals("0.01"));

		// intermediately excess zeros
		assert ((new BigRational(1, 101)).toStringDotRelative(1).equals("0.01"));
		assert ((new BigRational("0.0099999")).toStringDotRelative(1).equals("0.01"));

		// exponent representation

		assert ((new BigRational("1234.5678")).toStringExponent(8).equals("1.2345678E3"));
		assert ((new BigRational("1234.5678")).toStringExponent(4).equals("1.235E3"));
		assert ((new BigRational("1234.5678")).toStringExponent(2).equals("1.2E3"));
		assert ((new BigRational("1234.5678")).toStringExponent(1).equals("1E3"));
		assert ((new BigRational("1234.5678")).toStringExponent(0).equals("0"));
		assert ((new BigRational("0.00012345678")).toStringExponent(8).equals("1.2345678E-4"));
		assert ((new BigRational("0.00012345678")).toStringExponent(4).equals("1.235E-4"));
		assert ((new BigRational("0.00012345678")).toStringExponent(2).equals("1.2E-4"));
		assert ((new BigRational("0.00012345678")).toStringExponent(1).equals("1E-4"));
		assert ((new BigRational("0.00012345678")).toStringExponent(0).equals("0"));
		...
		assert ((new BigRational("1234")).toStringExponent(8).equals("1.234E3"));
		assert ((new BigRational("1234")).toStringExponent(4).equals("1.234E3"));
		assert ((new BigRational("1234")).toStringExponent(2).equals("1.2E3"));
		assert ((new BigRational("1234")).toStringExponent(1).equals("1E3"));
		assert ((new BigRational("1234")).toStringExponent(0).equals("0"));
		...
		assert (BigRational.valueOf("2E222").toStringExponent(4).equals("2E222"));
		assert (BigRational.valueOf("-2E222").toStringExponent(4).equals("-2E222"));
		assert (BigRational.valueOf("2E-222").toStringExponent(4).equals("2E-222"));
		assert (BigRational.valueOf("-2E-222").toStringExponent(4).equals("-2E-222"));

		// non-default radix
		assert (new BigRational("2E222", 5).toStringExponent(4, 5).equals("2E222"));
		assert (new BigRational("-2E222", 5).toStringExponent(4, 5).equals("-2E222"));
		assert (new BigRational("2E-222", 5).toStringExponent(4, 5).equals("2E-222"));
		assert (new BigRational("-2E-222", 5).toStringExponent(4, 5).equals("-2E-222"));

		// note: some of the arithmetic long forms not tested as well.

		// "BigRational add"
		assert ((new BigRational(3, 5)).add(new BigRational(7, 11)).toString().equals("68/55"));
		assert ((new BigRational(3, 5)).add(new BigRational(-7, 11)).toString().equals("-2/55"));
		assert ((new BigRational(-3, 5)).add(new BigRational(7, 11)).toString().equals("2/55"));
		assert ((new BigRational(-3, 5)).add(new BigRational(-7, 11)).toString().equals("-68/55"));
		// same denominator
		assert ((new BigRational(3, 5)).add(new BigRational(1, 5)).toString().equals("4/5"));
		// with integers
		assert ((new BigRational(3, 5)).add(new BigRational(1)).toString().equals("8/5"));
		assert ((new BigRational(2)).add(new BigRational(3, 5)).toString().equals("13/5"));
		// zero
		assert ((new BigRational(3, 5)).add(BigRational.ZERO).toString().equals("3/5"));
		assert (BigRational.ZERO.add(new BigRational(3, 5)).toString().equals("3/5"));

		// "BigRational subtract"
		assert ((new BigRational(3, 5)).subtract(new BigRational(7, 11)).toString().equals("-2/55"));
		assert ((new BigRational(3, 5)).subtract(new BigRational(-7, 11)).toString().equals("68/55"));
		assert ((new BigRational(-3, 5)).subtract(new BigRational(7, 11)).toString().equals("-68/55"));
		assert ((new BigRational(-3, 5)).subtract(new BigRational(-7, 11)).toString().equals("2/55"));
		// same denominator
		assert ((new BigRational(3, 5)).subtract(new BigRational(1, 5)).toString().equals("2/5"));
		// with integers
		assert ((new BigRational(3, 5)).subtract(new BigRational(1)).toString().equals("-2/5"));
		assert ((new BigRational(2)).subtract(new BigRational(3, 5)).toString().equals("7/5"));
		// zero
		assert ((new BigRational(3, 5)).subtract(BigRational.ZERO).toString().equals("3/5"));
		assert (BigRational.ZERO.subtract(new BigRational(3, 5)).toString().equals("-3/5"));

		// normalization, e.g after subtract
		// "BigRational normalization"
		assert ((new BigRational(7, 5)).subtract(new BigRational(2, 5)).compareTo(1) == 0);
		assert ((new BigRational(7, 5)).subtract(new BigRational(7, 5)).compareTo(0) == 0);
		assert ((new BigRational(7, 5)).subtract(new BigRational(12, 5)).compareTo(-1) == 0);

		// "BigRational multiply"
		assert ((new BigRational(3, 5)).multiply(new BigRational(7, 11)).toString().equals("21/55"));
		assert ((new BigRational(3, 5)).multiply(new BigRational(-7, 11)).toString().equals("-21/55"));
		assert ((new BigRational(-3, 5)).multiply(new BigRational(7, 11)).toString().equals("-21/55"));
		assert ((new BigRational(-3, 5)).multiply(new BigRational(-7, 11)).toString().equals("21/55"));
		assert ((new BigRational(3, 5)).multiply(7).toString().equals("21/5"));
		assert ((new BigRational(-3, 5)).multiply(7).toString().equals("-21/5"));
		assert ((new BigRational(3, 5)).multiply(-7).toString().equals("-21/5"));
		assert ((new BigRational(-3, 5)).multiply(-7).toString().equals("21/5"));
		...
		// "BigRational multiply"
		assert ((new BigRational(3, 5)).multiply(new BigRational(7, 1)).toString().equals("21/5"));
		assert ((new BigRational(3, 5)).multiply(new BigRational(1, 7)).toString().equals("3/35"));
		assert ((new BigRational(3, 1)).multiply(new BigRational(7, 11)).toString().equals("21/11"));
		assert ((new BigRational(3, 5)).multiply(new BigRational(0)).toString().equals("0"));
		assert ((new BigRational(0)).multiply(new BigRational(3, 5)).toString().equals("0"));
		assert ((new BigRational(3, 5)).multiply(new BigRational(1)).toString().equals("3/5"));
		assert ((new BigRational(3, 5)).multiply(new BigRational(-1)).toString().equals("-3/5"));
		assert ((new BigRational(3, 5)).multiply(new BigRational(-1, 3)).toString().equals("-1/5"));

		// special cases (zeroing, negating)
		assert (BigRational.ZERO.multiply(BigRational.ZERO).equals(BigRational.ZERO));
		assert (BigRational.ZERO.multiply(BigRational.ONE).equals(BigRational.ZERO));
		assert (BigRational.ONE.multiply(BigRational.ZERO).equals(BigRational.ZERO));
		assert (BigRational.ZERO.multiply(BigRational.MINUS_ONE).equals(BigRational.ZERO));
		assert (BigRational.MINUS_ONE.multiply(BigRational.ZERO).equals(BigRational.ZERO));
		assert (BigRational.ZERO.multiply(new BigRational(5)).equals(BigRational.ZERO));
		assert ((new BigRational(5)).multiply(BigRational.ZERO).equals(BigRational.ZERO));
		assert (BigRational.ZERO.multiply(new BigRational(-5)).equals(BigRational.ZERO));
		assert ((new BigRational(-5)).multiply(BigRational.ZERO).equals(BigRational.ZERO));
		assert (BigRational.ONE.multiply(BigRational.ONE).equals(BigRational.ONE));
		assert (BigRational.ONE.multiply(BigRational.MINUS_ONE).equals(BigRational.MINUS_ONE));
		assert (BigRational.MINUS_ONE.multiply(BigRational.ONE).equals(BigRational.MINUS_ONE));
		assert (BigRational.ONE.multiply(new BigRational(5)).equals(new BigRational(5)));
		assert ((new BigRational(5)).multiply(BigRational.ONE).equals(new BigRational(5)));
		assert (BigRational.ONE.multiply(new BigRational(-5)).equals(new BigRational(-5)));
		assert ((new BigRational(-5)).multiply(BigRational.ONE).equals(new BigRational(-5)));
		assert (BigRational.MINUS_ONE.multiply(BigRational.MINUS_ONE).equals(BigRational.ONE));
		assert (BigRational.MINUS_ONE.multiply(new BigRational(5)).equals(new BigRational(-5)));
		assert ((new BigRational(5)).multiply(BigRational.MINUS_ONE).equals(new BigRational(-5)));
		assert (BigRational.MINUS_ONE.multiply(new BigRational(-5)).equals(new BigRational(5)));
		assert ((new BigRational(-5)).multiply(BigRational.MINUS_ONE).equals(new BigRational(5)));

		// "BigRational divide"
		assert ((new BigRational(3, 5)).divide(new BigRational(7, 11)).toString().equals("33/35"));
		assert ((new BigRational(3, 5)).divide(new BigRational(-7, 11)).toString().equals("-33/35"));
		assert ((new BigRational(-3, 5)).divide(new BigRational(7, 11)).toString().equals("-33/35"));
		assert ((new BigRational(-3, 5)).divide(new BigRational(-7, 11)).toString().equals("33/35"));
		assert ((new BigRational(3, 5)).divide(7).toString().equals("3/35"));
		assert ((new BigRational(-3, 5)).divide(7).toString().equals("-3/35"));
		assert ((new BigRational(3, 5)).divide(-7).toString().equals("-3/35"));
		assert ((new BigRational(-3, 5)).divide(-7).toString().equals("3/35"));
		...
		// "BigRational power"
		assert ((new BigRational(3, 5)).power(7).toString().equals("2187/78125"));
		assert ((new BigRational(-3, 5)).power(7).toString().equals("-2187/78125"));
		assert ((new BigRational(3, 5)).power(-7).toString().equals("78125/2187"));
		assert ((new BigRational(-3, 5)).power(-7).toString().equals("-78125/2187"));
		assert ((new BigRational(3, 5)).power(6).toString().equals("729/15625"));
		assert ((new BigRational(-3, 5)).power(6).toString().equals("729/15625"));
		assert ((new BigRational(3, 5)).power(0).toString().equals("1"));
		assert ((new BigRational(-3, 5)).power(0).toString().equals("1"));
		assert ((new BigRational(0)).power(1).toString().equals("0"));
		assert ((new BigRational(1)).power(0).toString().equals("1"));

		assert ((new BigRational(3, 5)).power(0).equals(BigRational.ONE));
		assert ((new BigRational(3, 5)).power(1).equals(new BigRational(3, 5)));
		assert ((new BigRational(3, 5)).power(-1).equals((new BigRational(3, 5)).invert()));
		assert ((new BigRational(2)).power(2).equals(new BigRational(4)));

		// special cases
		assert (BigRational.ZERO.power(1).equals(BigRational.ZERO));
		assert (BigRational.ZERO.power(2).equals(BigRational.ZERO));
		assert (BigRational.ZERO.power(3).equals(BigRational.ZERO));
		assert (BigRational.ZERO.power(4).equals(BigRational.ZERO));
		assert (BigRational.ONE.power(0).equals(BigRational.ONE));
		assert (BigRational.ONE.power(1).equals(BigRational.ONE));
		assert (BigRational.ONE.power(2).equals(BigRational.ONE));
		assert (BigRational.ONE.power(3).equals(BigRational.ONE));
		assert (BigRational.ONE.power(4).equals(BigRational.ONE));
		assert (BigRational.ONE.power(-1).equals(BigRational.ONE));
		assert (BigRational.ONE.power(-2).equals(BigRational.ONE));
		assert (BigRational.ONE.power(-3).equals(BigRational.ONE));
		assert (BigRational.ONE.power(-4).equals(BigRational.ONE));
		assert (BigRational.MINUS_ONE.power(0).equals(BigRational.ONE));
		assert (BigRational.MINUS_ONE.power(1).equals(BigRational.MINUS_ONE));
		assert (BigRational.MINUS_ONE.power(2).equals(BigRational.ONE));
		assert (BigRational.MINUS_ONE.power(3).equals(BigRational.MINUS_ONE));
		...
		assert ((new BigRational(5)).remainder(new BigRational(3)).equals(new BigRational(2)));
		assert ((new BigRational(-5)).remainder(new BigRational(3)).equals(new BigRational(-2)));
		assert ((new BigRational(5)).remainder(new BigRational(-3)).equals(new BigRational(2)));
		assert ((new BigRational(-5)).remainder(new BigRational(-3)).equals(new BigRational(-2)));
		assert ((new BigRational(0)).remainder(new BigRational(1)).equals(new BigRational(0)));
		assert ((new BigRational("5.6")).remainder(new BigRational("1.8")).equals(new BigRational(1, 5)));
		assert ((new BigRational("-5.6")).remainder(new BigRational("1.8")).equals(new BigRational(-1, 5)));
		assert ((new BigRational("5.6")).remainder(new BigRational("-1.8")).equals(new BigRational(1, 5)));
		assert ((new BigRational("-5.6")).remainder(new BigRational("-1.8")).equals(new BigRational(-1, 5)));
		assert ((new BigRational("1")).remainder(new BigRational("0.13")).equals(new BigRational("0.09")));
		...
		// "BigRational modulus"
		assert ((new BigRational(5)).modulus(new BigRational(3)).equals(new BigRational(2)));
		assert ((new BigRational(-5)).modulus(new BigRational(3)).equals(new BigRational(1)));
		assert ((new BigRational(5)).modulus(new BigRational(-3)).equals(new BigRational(-1)));
		assert ((new BigRational(-5)).modulus(new BigRational(-3)).equals(new BigRational(-2)));
		assert ((new BigRational(0)).modulus(new BigRational(1)).equals(new BigRational(0)));
		assert ((new BigRational("5.6")).modulus(new BigRational("1.8")).equals(new BigRational(1, 5)));
		assert ((new BigRational("-5.6")).modulus(new BigRational("1.8")).equals(new BigRational(8, 5)));
		assert ((new BigRational("5.6")).modulus(new BigRational("-1.8")).equals(new BigRational(-8, 5)));
		assert ((new BigRational("-5.6")).modulus(new BigRational("-1.8")).equals(new BigRational(-1, 5)));
		assert ((new BigRational("1")).modulus(new BigRational("0.13")).equals(new BigRational("0.09")));
		...
		// "BigRational signum"
		assert ((new BigRational(0)).signum() == 0);
		assert ((new BigRational(1)).signum() == 1);
		assert ((new BigRational(-1)).signum() == -1);
		assert ((new BigRational(2)).signum() == 1);
		assert ((new BigRational(-2)).signum() == -1);
		assert ((new BigRational(3, 5)).signum() == 1);
		assert ((new BigRational(-3, 5)).signum() == -1);

		// "BigRational absolute"
		assert ((new BigRational(0)).absolute().toString().equals("0"));
		assert ((new BigRational(3, 5)).absolute().toString().equals("3/5"));
		assert ((new BigRational(-3, 5)).absolute().toString().equals("3/5"));
		assert ((new BigRational(1)).absolute().toString().equals("1"));
		assert ((new BigRational(-1)).absolute().toString().equals("1"));
		assert (BigRational.ZERO.absolute().equals(BigRational.ZERO));
		assert (BigRational.ONE.absolute().equals(BigRational.ONE));
		assert (BigRational.MINUS_ONE.absolute().equals(BigRational.ONE));

		// "BigRational negate"
		assert ((new BigRational(0)).negate().toString().equals("0"));
		assert ((new BigRational(1)).negate().toString().equals("-1"));
		assert ((new BigRational(-1)).negate().toString().equals("1"));
		assert ((new BigRational(3, 5)).negate().toString().equals("-3/5"));
		assert ((new BigRational(-3, 5)).negate().toString().equals("3/5"));
		assert (BigRational.ZERO.negate().equals(BigRational.ZERO));
		assert (BigRational.ONE.negate().equals(BigRational.MINUS_ONE));
		assert (BigRational.MINUS_ONE.negate().equals(BigRational.ONE));

		// "BigRational invert"
		assert ((new BigRational(3, 5)).invert().toString().equals("5/3"));
		assert ((new BigRational(-3, 5)).invert().toString().equals("-5/3"));
		assert ((new BigRational(11, 7)).invert().toString().equals("7/11"));
		assert ((new BigRational(-11, 7)).invert().toString().equals("-7/11"));
		assert ((new BigRational(1)).invert().toString().equals("1"));
		assert ((new BigRational(-1)).invert().toString().equals("-1"));
		assert ((new BigRational(2)).invert().toString().equals("1/2"));
		assert (BigRational.ONE.invert().equals(BigRational.ONE));
		assert (BigRational.MINUS_ONE.invert().equals(BigRational.MINUS_ONE));
		...
		// "BigRational minimum"
		assert ((new BigRational(3, 5)).minimum(new BigRational(7, 11)).toString().equals("3/5"));
		assert ((new BigRational(-3, 5)).minimum(new BigRational(7, 11)).toString().equals("-3/5"));
		assert ((new BigRational(3, 5)).minimum(new BigRational(-7, 11)).toString().equals("-7/11"));
		assert ((new BigRational(-3, 5)).minimum(new BigRational(-7, 11)).toString().equals("-7/11"));
		assert ((new BigRational(7, 11)).minimum(new BigRational(3, 5)).toString().equals("3/5"));

		// "BigRational maximum"
		assert ((new BigRational(3, 5)).maximum(new BigRational(7, 11)).toString().equals("7/11"));
		assert ((new BigRational(-3, 5)).maximum(new BigRational(7, 11)).toString().equals("7/11"));
		assert ((new BigRational(3, 5)).maximum(new BigRational(-7, 11)).toString().equals("3/5"));
		assert ((new BigRational(-3, 5)).maximum(new BigRational(-7, 11)).toString().equals("-3/5"));
		assert ((new BigRational(7, 11)).maximum(new BigRational(3, 5)).toString().equals("7/11"));

		// equals()

		// "BigRational [polymorph] equals"
		assert (BigRational.ONE.equals(BigRational.ONE));
		assert (BigRational.ONE.equals(new BigRational(3, 3)));
		assert (BigRational.ONE.equals(new BigRational(987654, 987654)));
		assert (!BigRational.ONE.equals(new BigRational(987653, 987654)));
		assert (BigRational.ZERO.equals(new BigRational(0, 987654)));
		assert (BigRational.MINUS_ONE.equals(new BigRational(-987654, 987654)));
		assert (!BigRational.MINUS_ONE.equals(new BigRational(-987653, 987654)));
		assert ((new BigRational(3, 5)).equals(new BigRational(3, 5)));
		assert (!(new BigRational(3, 5)).equals(new BigRational(5, 3)));
		assert (BigRational.ZERO.equals(BigRational.ZERO));
		assert (BigRational.ONE.equals(BigRational.ONE));
		assert (BigRational.MINUS_ONE.equals(BigRational.MINUS_ONE));
		assert (!BigRational.ZERO.equals(BigRational.ONE));
		assert (!BigRational.ONE.equals(BigRational.ZERO));
		assert (!BigRational.ONE.equals(BigRational.MINUS_ONE));
		assert (!BigRational.MINUS_ONE.equals(BigRational.ONE));

		// following tests address things that changed from earlier version;
		// i.e. will fail with that version
		assert (!BigRational.ZERO.equals(new Integer(0)));
		assert (!(new BigRational(3)).equals(new Integer(3)));
		assert (!BigRational.ZERO.equals(new Long(0)));
		assert (!(new BigRational(3)).equals(new Long(3)));
		assert (!BigRational.ZERO.equals("0"));
		assert (!(new BigRational(3)).equals("3"));

		assert (!(new Integer(0)).equals(BigRational.ZERO));
		assert (!(new Long(0)).equals(BigRational.ZERO));
		assert (!(new String("0")).equals(BigRational.ZERO));

		assert (!BigRational.ONE.equals(new RuntimeException()));
		assert (!(new RuntimeException()).equals(BigRational.ONE));

		// "BigRational hash code"
		assert ((new BigRational(3, 5)).hashCode() == (new BigRational(6, 10)).hashCode());
		...
		// assert (BigRational.ONE.hashCode() != BigRational.ZERO.hashCode());
		// assert ((new BigRational(3, 5)).hashCode() != (new BigRational(5, 3)).hashCode());
		// assert ((new BigRational(3, 5)).hashCode() != (new BigRational(-3, 5)).hashCode());
		// assert ((new BigRational(4)).hashCode() != (new BigRational(8)).hashCode());

		// "BigRational compare to"
		assert ((new BigRational(3, 5)).compareTo(new BigRational(3, 5)) == 0);
		assert ((new BigRational(3, 5)).compareTo(new BigRational(5, 3)) == -1);
		assert ((new BigRational(5, 3)).compareTo(new BigRational(3, 5)) == 1);
		assert ((new BigRational(3, 5)).compareTo(new BigRational(-5, 3)) == 1);
		assert ((new BigRational(-5, 3)).compareTo(new BigRational(3, 5)) == -1);

		assert (BigRational.ZERO.compareTo(BigRational.ZERO) == 0);
		assert (BigRational.ZERO.compareTo(BigRational.ONE) == -1);
		assert (BigRational.ONE.compareTo(BigRational.ZERO) == 1);
		assert (BigRational.ZERO.compareTo(BigRational.MINUS_ONE) == 1);
		assert (BigRational.MINUS_ONE.compareTo(BigRational.ZERO) == -1);
		assert (BigRational.ONE.compareTo(BigRational.MINUS_ONE) == 1);
		assert (BigRational.MINUS_ONE.compareTo(BigRational.ONE) == -1);
		assert (BigRational.ONE.compareTo(BigRational.ONE) == 0);

		// "BigRational polymorph compare to"
		assert ((new BigRational(3, 5)).compareTo((Object)new BigRational(3, 5)) == 0);
		assert ((new BigRational(3, 5)).compareTo((Object)new BigRational(5, 3)) == -1);
		assert ((new BigRational(5, 3)).compareTo((Object)new BigRational(3, 5)) == 1);
		assert ((new BigRational(3, 5)).compareTo((Object)new BigRational(-5, 3)) == 1);
		assert ((new BigRational(-5, 3)).compareTo((Object)new BigRational(3, 5)) == -1);

		// "BigRational small type compare to"
		assert ((new BigRational(3, 5)).compareTo(0) != 0);
		assert ((new BigRational(0)).compareTo(0) == 0);
		assert ((new BigRational(1)).compareTo(1) == 0);
		assert ((new BigRational(2)).compareTo(2) == 0);
		assert ((new BigRational(3, 5)).compareTo(new Long(0)) != 0);
		assert ((new BigRational(1)).compareTo(new Long(1)) == 0);
		assert ((new BigRational(1)).compareTo(new Integer(1)) == 0);

		// "BigRational long/int value"
		assert ((new BigRational(7)).longValue() == 7);
		assert ((new BigRational(-7)).intValue() == -7);
		assert (BigRational.ZERO.longValue() == 0);
		assert (BigRational.MINUS_ONE.longValue() == -1);
		assert (BigRational.MINUS_ONE.intValue() == -1);
		assert (BigRational.MINUS_ONE.shortValue() == -1);
		assert (BigRational.MINUS_ONE.byteValue() == -1);
		...
		assert ((new BigRational("23.49")).round(BigRational.ROUND_HALF_DOWN).toString().equals("23"));
		assert ((new BigRational("23.49")).round(BigRational.ROUND_HALF_CEILING).toString().equals("23"));
		assert ((new BigRational("23.49")).round(BigRational.ROUND_HALF_FLOOR).toString().equals("23"));
		assert ((new BigRational("23.49")).round(BigRational.ROUND_HALF_EVEN).toString().equals("23"));
		assert ((new BigRational("23.49")).round(BigRational.ROUND_HALF_ODD).toString().equals("23"));

		assert ((new BigRational("-23.49")).round(BigRational.ROUND_UP).toString().equals("-24"));
		assert ((new BigRational("-23.49")).round(BigRational.ROUND_DOWN).toString().equals("-23"));
		assert ((new BigRational("-23.49")).round(BigRational.ROUND_CEILING).toString().equals("-23"));
		assert ((new BigRational("-23.49")).round(BigRational.ROUND_FLOOR).toString().equals("-24"));
		assert ((new BigRational("-23.49")).round(BigRational.ROUND_HALF_UP).toString().equals("-23"));
		assert ((new BigRational("-23.49")).round(BigRational.ROUND_HALF_DOWN).toString().equals("-23"));
		assert ((new BigRational("-23.49")).round(BigRational.ROUND_HALF_CEILING).toString().equals("-23"));
		assert ((new BigRational("-23.49")).round(BigRational.ROUND_HALF_FLOOR).toString().equals("-23"));
		...
		assert ((new BigRational("23.51")).round(BigRational.ROUND_HALF_DOWN).toString().equals("24"));
		assert ((new BigRational("23.51")).round(BigRational.ROUND_HALF_CEILING).toString().equals("24"));
		assert ((new BigRational("23.51")).round(BigRational.ROUND_HALF_FLOOR).toString().equals("24"));
		assert ((new BigRational("23.51")).round(BigRational.ROUND_HALF_EVEN).toString().equals("24"));
		assert ((new BigRational("23.51")).round(BigRational.ROUND_HALF_ODD).toString().equals("24"));

		assert ((new BigRational("-23.51")).round(BigRational.ROUND_UP).toString().equals("-24"));
		assert ((new BigRational("-23.51")).round(BigRational.ROUND_DOWN).toString().equals("-23"));
		assert ((new BigRational("-23.51")).round(BigRational.ROUND_CEILING).toString().equals("-23"));
		assert ((new BigRational("-23.51")).round(BigRational.ROUND_FLOOR).toString().equals("-24"));
		assert ((new BigRational("-23.51")).round(BigRational.ROUND_HALF_UP).toString().equals("-24"));
		assert ((new BigRational("-23.51")).round(BigRational.ROUND_HALF_DOWN).toString().equals("-24"));
		assert ((new BigRational("-23.51")).round(BigRational.ROUND_HALF_CEILING).toString().equals("-24"));
		assert ((new BigRational("-23.51")).round(BigRational.ROUND_HALF_FLOOR).toString().equals("-24"));
		...
		assert ((new BigRational("23.5")).round(BigRational.ROUND_HALF_DOWN).toString().equals("23"));
		assert ((new BigRational("23.5")).round(BigRational.ROUND_HALF_CEILING).toString().equals("24"));
		assert ((new BigRational("23.5")).round(BigRational.ROUND_HALF_FLOOR).toString().equals("23"));
		assert ((new BigRational("23.5")).round(BigRational.ROUND_HALF_EVEN).toString().equals("24"));
		assert ((new BigRational("23.5")).round(BigRational.ROUND_HALF_ODD).toString().equals("23"));

		assert ((new BigRational("-23.5")).round(BigRational.ROUND_UP).toString().equals("-24"));
		assert ((new BigRational("-23.5")).round(BigRational.ROUND_DOWN).toString().equals("-23"));
		assert ((new BigRational("-23.5")).round(BigRational.ROUND_CEILING).toString().equals("-23"));
		assert ((new BigRational("-23.5")).round(BigRational.ROUND_FLOOR).toString().equals("-24"));
		assert ((new BigRational("-23.5")).round(BigRational.ROUND_HALF_UP).toString().equals("-24"));
		assert ((new BigRational("-23.5")).round(BigRational.ROUND_HALF_DOWN).toString().equals("-23"));
		assert ((new BigRational("-23.5")).round(BigRational.ROUND_HALF_CEILING).toString().equals("-23"));
		assert ((new BigRational("-23.5")).round(BigRational.ROUND_HALF_FLOOR).toString().equals("-24"));
		assert ((new BigRational("-23.5")).round(BigRational.ROUND_HALF_EVEN).toString().equals("-24"));
		assert ((new BigRational("-23.5")).round(BigRational.ROUND_HALF_ODD).toString().equals("-23"));
		...
		assert ((new BigRational("22")).round(BigRational.ROUND_HALF_DOWN).toString().equals("22"));
		assert ((new BigRational("22")).round(BigRational.ROUND_HALF_CEILING).toString().equals("22"));
		assert ((new BigRational("22")).round(BigRational.ROUND_HALF_FLOOR).toString().equals("22"));
		assert ((new BigRational("22")).round(BigRational.ROUND_HALF_EVEN).toString().equals("22"));
		...
		assert ((new BigRational("23")).round(BigRational.ROUND_UNNECESSARY).toString().equals("23"));
		assert ((new BigRational("-23")).round(BigRational.ROUND_UNNECESSARY).toString().equals("-23"));
		...
		// round special cases
		assert (BigRational.ZERO.round().equals(BigRational.ZERO));
		assert (BigRational.ZERO.round(BigRational.ROUND_UNNECESSARY).equals(BigRational.ZERO));

		// "BigRational integer part"
		assert ((new BigRational("56.8")).integerPart().toString().equals("56"));
		assert ((new BigRational("-56.8")).integerPart().toString().equals("-56"));
		assert ((new BigRational("0.8")).integerPart().toString().equals("0"));
		assert ((new BigRational("-0.8")).integerPart().toString().equals("0"));

		// "BigRational fractional part"
		assert ((new BigRational("56.8")).fractionalPart().equals(new BigRational("0.8")));
		assert ((new BigRational("-56.8")).fractionalPart().equals(new BigRational("-0.8")));
		assert ((new BigRational("0.8")).fractionalPart().equals(new BigRational("0.8")));
		assert ((new BigRational("-0.8")).fractionalPart().equals(new BigRational("-0.8")));

		// "BigRational integer and fractional part"
		assert ((new BigRational("56.8")).integerAndFractionalPart()[0].equals(new BigRational("56")));
		assert ((new BigRational("56.8")).integerAndFractionalPart()[1].equals(new BigRational("0.8")));
		assert ((new BigRational("-56.8")).integerAndFractionalPart()[0].equals(new BigRational("-56")));
		assert ((new BigRational("-56.8")).integerAndFractionalPart()[1].equals(new BigRational("-0.8")));

		// sanity/harmlessness of using internal constants of commonly used numbers
		// scanned in by commonly used radixes
		assert ((new BigRational("0", 2)).toString(2).equals("0"));
		assert ((new BigRational("0", 10)).toString(10).equals("0"));
		assert ((new BigRational("0", 16)).toString(16).equals("0"));
		assert ((new BigRational("1", 2)).toString(2).equals("1"));
		assert ((new BigRational("1", 10)).toString(10).equals("1"));
		assert ((new BigRational("1", 16)).toString(16).equals("1"));
		assert ((new BigRational("-1", 2)).toString(2).equals("-1"));
		assert ((new BigRational("-1", 10)).toString(10).equals("-1"));
		assert ((new BigRational("-1", 16)).toString(16).equals("-1"));
		assert ((new BigRational("10", 2)).toString(2).equals("10"));
		assert ((new BigRational("10", 10)).toString(10).equals("10"));
		assert ((new BigRational("10", 16)).toString(16).equals("10"));

		// more constants
		assert ((new BigRational("2", 10)).toString(10).equals("2"));
		assert ((new BigRational("2", 16)).toString(16).equals("2"));
		assert ((new BigRational("-2", 10)).toString(10).equals("-2"));
		assert ((new BigRational("-2", 16)).toString(16).equals("-2"));
		assert ((new BigRational("16", 10)).toString(10).equals("16"));
		assert ((new BigRational("16", 16)).toString(16).equals("16"));

		// floating point
		// remember: [IEEE 754] floats are exact where the quotient is a power of two

		assert (BigRational.valueOf(0.0).equals(BigRational.ZERO));
		assert (BigRational.valueOf(1.0).equals(BigRational.ONE));
		assert (BigRational.valueOf(-1.0).equals(BigRational.MINUS_ONE));
		...
		assert (BigRational.ZERO.doubleValue() == 0.0);
		assert (BigRational.ONE.doubleValue() == 1.0);
		assert (BigRational.MINUS_ONE.doubleValue() == -1.0);

		assert (BigRational.valueOf(2).doubleValue() == 2.0);
		assert (BigRational.valueOf(-2).doubleValue() == -2.0);
		assert (BigRational.valueOf(4).doubleValue() == 4.0);
		assert (BigRational.valueOf(-4).doubleValue() == -4.0);
		assert (BigRational.valueOf(16).doubleValue() == 16.0);
		assert (BigRational.valueOf(-16).doubleValue() == -16.0);

		assert (BigRational.valueOf(3).doubleValue() == 3.0);
		assert (BigRational.valueOf(-3).doubleValue() == -3.0);
		assert (BigRational.valueOf(6).doubleValue() == 6.0);
		assert (BigRational.valueOf(-6).doubleValue() == -6.0);
		assert (BigRational.valueOf(12).doubleValue() == 12.0);
		assert (BigRational.valueOf(-12).doubleValue() == -12.0);

		assert ((new BigRational(1, 2)).doubleValue() == 0.5);
		assert ((new BigRational(-1, 2)).doubleValue() == -0.5);
		assert ((new BigRational(1, 4)).doubleValue() == 0.25);
		assert ((new BigRational(-1, 4)).doubleValue() == -0.25);
		assert ((new BigRational(1, 16)).doubleValue() == 0.0625);
		assert ((new BigRational(-1, 16)).doubleValue() == -0.0625);

		assert ((new BigRational(3, 2)).doubleValue() == 1.5);
		assert ((new BigRational(-3, 2)).doubleValue() == -1.5);
		assert ((new BigRational(3, 4)).doubleValue() == 0.75);
		assert ((new BigRational(-3, 4)).doubleValue() == -0.75);
		assert ((new BigRational(3, 8)).doubleValue() == 0.375);
		assert ((new BigRational(-3, 8)).doubleValue() == -0.375);

		// conversion forth and back

		assert (BigRational.valueOf(0.0).doubleValue() == 0.0);
		assert (BigRational.valueOf(1.0).doubleValue() == 1.0);
		assert (BigRational.valueOf(-1.0).doubleValue() == -1.0);
		assert (BigRational.valueOf(2.0).doubleValue() == 2.0);
		assert (BigRational.valueOf(-2.0).doubleValue() == -2.0);

		// maximal and minimal values, and near there
		assert (BigRational.valueOf(Double.MAX_VALUE).doubleValue() == Double.MAX_VALUE);
		assert (BigRational.valueOf(-Double.MAX_VALUE).doubleValue() == -Double.MAX_VALUE);
		assert (BigRational.valueOf(Double.MAX_VALUE / 16).doubleValue() == Double.MAX_VALUE / 16);
		assert (BigRational.valueOf(-Double.MAX_VALUE / 16).doubleValue() == -Double.MAX_VALUE / 16);
		// [subnormal value]
		assert (BigRational.valueOf(Double.MIN_VALUE).doubleValue() == Double.MIN_VALUE);
		assert (BigRational.valueOf(-Double.MIN_VALUE).doubleValue() == -Double.MIN_VALUE);
		assert (BigRational.valueOf(Double.MIN_VALUE * 16).doubleValue() == Double.MIN_VALUE * 16);
		assert (BigRational.valueOf(-Double.MIN_VALUE * 16).doubleValue() == -Double.MIN_VALUE * 16);

		// overflow
		assert (BigRational.valueOf(Double.MAX_VALUE).multiply(2).doubleValue() == Double.POSITIVE_INFINITY);
		assert (BigRational.valueOf(Double.MAX_VALUE).multiply(4).doubleValue() == Double.POSITIVE_INFINITY);
		assert (BigRational.valueOf(Double.MAX_VALUE).multiply(valueOf(1.2)).doubleValue() == Double.POSITIVE_INFINITY);
		assert (BigRational.valueOf(Double.MAX_VALUE).multiply(16).doubleValue() == Double.POSITIVE_INFINITY);
		assert (BigRational.valueOf(-Double.MAX_VALUE).multiply(2).doubleValue() == Double.NEGATIVE_INFINITY);
		assert (BigRational.valueOf(-Double.MAX_VALUE).multiply(4).doubleValue() == Double.NEGATIVE_INFINITY);
		assert (BigRational.valueOf(-Double.MAX_VALUE).multiply(valueOf(1.2)).doubleValue() == Double.NEGATIVE_INFINITY);
		assert (BigRational.valueOf(-Double.MAX_VALUE).multiply(16).doubleValue() == Double.NEGATIVE_INFINITY);

		// underflow
		// note that the (double)x==(double)y test yields true for 0.0==-0.0
		assert (BigRational.valueOf(Double.MIN_VALUE).divide(2).doubleValue() == 0.0);
		assert (BigRational.valueOf(Double.MIN_VALUE).divide(4).doubleValue() == 0.0);
		...
		assert (String.valueOf(BigRational.valueOf(Double.MIN_VALUE).divide(2).doubleValue()).equals("0.0"));
		assert (String.valueOf(BigRational.valueOf(Double.MIN_VALUE).divide(4).doubleValue()).equals("0.0"));
		assert (String.valueOf(BigRational.valueOf(Double.MIN_VALUE).divide(valueOf(1.2)).doubleValue()).equals("0.0"));
		assert (String.valueOf(BigRational.valueOf(Double.MIN_VALUE).divide(16).doubleValue()).equals("0.0"));
		assert (String.valueOf(BigRational.valueOf(-Double.MIN_VALUE).divide(2).doubleValue()).equals("-0.0"));
		assert (String.valueOf(BigRational.valueOf(-Double.MIN_VALUE).divide(4).doubleValue()).equals("-0.0"));
		assert (String.valueOf(BigRational.valueOf(-Double.MIN_VALUE).divide(valueOf(1.2)).doubleValue()).equals("-0.0"));
		assert (String.valueOf(BigRational.valueOf(-Double.MIN_VALUE).divide(16).doubleValue()).equals("-0.0"));
		...
		// ulp
		assert (BigRational.valueOf(1.0 - StrictMath.ulp(1.0)).doubleValue() == 1.0 - StrictMath.ulp(1.0));
		assert (BigRational.valueOf(1.0 + StrictMath.ulp(1.0)).doubleValue() == 1.0 + StrictMath.ulp(1.0));
		assert (BigRational.valueOf(1.0 - StrictMath.ulp(1.0) / 2).doubleValue() == 1.0 - StrictMath.ulp(1.0) / 2);
		assert (BigRational.valueOf(1.0 + StrictMath.ulp(1.0) / 2).doubleValue() == 1.0 + StrictMath.ulp(1.0) / 2);
		assert (BigRational.valueOf(1.0 - StrictMath.ulp(1.0) / 4).doubleValue() == 1.0 - StrictMath.ulp(1.0) / 4);
		assert (BigRational.valueOf(1.0 + StrictMath.ulp(1.0) / 4).doubleValue() == 1.0 + StrictMath.ulp(1.0) / 4);

		// mantissa rounding
		// a delta of ulp/4 is expected to be rounded
		assert (BigRational.valueOf(1.0).subtract(BigRational.valueOf(StrictMath.ulp(1.0)).divide(4)).doubleValue() == 1.0);
		assert (BigRational.valueOf(16.0).subtract(BigRational.valueOf(StrictMath.ulp(16.0)).divide(4)).doubleValue() == 16.0);
		...
		// assert (BigRational.valueOf(0x1P1023).subtract(BigRational.valueOf(StrictMath.ulp(0x1P1023)).divide(4)).doubleValue() == 0x1P1023);

		// more values in between [0 and max/min]
		assert (BigRational.valueOf(StrictMath.sqrt(Double.MAX_VALUE)).doubleValue() == StrictMath.sqrt(Double.MAX_VALUE));
		assert (BigRational.valueOf(StrictMath.pow(Double.MAX_VALUE, 0.2)).doubleValue() == StrictMath.pow(Double.MAX_VALUE, 0.2));
		assert (BigRational.valueOf(Double.MAX_VALUE).power(2).doubleValue() == Double.POSITIVE_INFINITY);
		assert (BigRational.valueOf(Double.MAX_VALUE).power(5).doubleValue() == Double.POSITIVE_INFINITY);
		assert (BigRational.valueOf(StrictMath.sqrt(Double.MIN_VALUE)).doubleValue() == StrictMath.sqrt(Double.MIN_VALUE));
		assert (BigRational.valueOf(StrictMath.pow(Double.MIN_VALUE, 0.2)).doubleValue() == StrictMath.pow(Double.MIN_VALUE, 0.2));
		assert (BigRational.valueOf(Double.MIN_VALUE).power(2).doubleValue() == 0.0);
		assert (BigRational.valueOf(Double.MIN_VALUE).power(5).doubleValue() == 0.0);
		...
		assert (BigRational.valueOf(StrictMath.E).doubleValue() == StrictMath.E);
		assert (BigRational.valueOf(StrictMath.PI).doubleValue() == StrictMath.PI);
		assert (BigRational.valueOf(StrictMath.pow(StrictMath.E, 2)).doubleValue() == StrictMath.pow(StrictMath.E, 2));
		assert (BigRational.valueOf(StrictMath.sqrt(StrictMath.E)).doubleValue() == StrictMath.sqrt(StrictMath.E));
		assert (BigRational.valueOf(StrictMath.pow(StrictMath.PI, 2)).doubleValue() == StrictMath.pow(StrictMath.PI, 2));
		assert (BigRational.valueOf(StrictMath.sqrt(StrictMath.PI)).doubleValue() == StrictMath.sqrt(StrictMath.PI));

		// same tests with single precision float

		assert (BigRational.valueOf(0.0f).equals(BigRational.ZERO));
		assert (BigRational.valueOf(1.0f).equals(BigRational.ONE));
		assert (BigRational.valueOf(-1.0f).equals(BigRational.MINUS_ONE));
		...
		assert (BigRational.ZERO.floatValue() == 0.0f);
		assert (BigRational.ONE.floatValue() == 1.0f);
		assert (BigRational.MINUS_ONE.floatValue() == -1.0f);

		assert (BigRational.valueOf(2).floatValue() == 2.0f);
		assert (BigRational.valueOf(-2).floatValue() == -2.0f);
		assert (BigRational.valueOf(4).floatValue() == 4.0f);
		assert (BigRational.valueOf(-4).floatValue() == -4.0f);
		assert (BigRational.valueOf(16).floatValue() == 16.0f);
		assert (BigRational.valueOf(-16).floatValue() == -16.0f);

		assert (BigRational.valueOf(3).floatValue() == 3.0f);
		assert (BigRational.valueOf(-3).floatValue() == -3.0f);
		assert (BigRational.valueOf(6).floatValue() == 6.0f);
		assert (BigRational.valueOf(-6).floatValue() == -6.0f);
		assert (BigRational.valueOf(12).floatValue() == 12.0f);
		assert (BigRational.valueOf(-12).floatValue() == -12.0f);

		assert ((new BigRational(1, 2)).floatValue() == 0.5f);
		assert ((new BigRational(-1, 2)).floatValue() == -0.5f);
		assert ((new BigRational(1, 4)).floatValue() == 0.25f);
		assert ((new BigRational(-1, 4)).floatValue() == -0.25f);
		assert ((new BigRational(1, 16)).floatValue() == 0.0625f);
		assert ((new BigRational(-1, 16)).floatValue() == -0.0625f);

		assert ((new BigRational(3, 2)).floatValue() == 1.5f);
		assert ((new BigRational(-3, 2)).floatValue() == -1.5f);
		assert ((new BigRational(3, 4)).floatValue() == 0.75f);
		assert ((new BigRational(-3, 4)).floatValue() == -0.75f);
		assert ((new BigRational(3, 8)).floatValue() == 0.375f);
		assert ((new BigRational(-3, 8)).floatValue() == -0.375f);

		// conversion forth and back

		assert (BigRational.valueOf(0.0f).floatValue() == 0.0f);
		assert (BigRational.valueOf(1.0f).floatValue() == 1.0f);
		assert (BigRational.valueOf(-1.0f).floatValue() == -1.0f);
		assert (BigRational.valueOf(2.0f).floatValue() == 2.0f);
		assert (BigRational.valueOf(-2.0f).floatValue() == -2.0f);

		// maximal and minimal values, and near there
		assert (BigRational.valueOf(Float.MAX_VALUE).floatValue() == Float.MAX_VALUE);
		assert (BigRational.valueOf(-Float.MAX_VALUE).floatValue() == -Float.MAX_VALUE);
		assert (BigRational.valueOf(Float.MAX_VALUE / 16).floatValue() == Float.MAX_VALUE / 16);
		assert (BigRational.valueOf(-Float.MAX_VALUE / 16).floatValue() == -Float.MAX_VALUE / 16);
		// [subnormal value]
		assert (BigRational.valueOf(Float.MIN_VALUE).floatValue() == Float.MIN_VALUE);
		assert (BigRational.valueOf(-Float.MIN_VALUE).floatValue() == -Float.MIN_VALUE);
		assert (BigRational.valueOf(Float.MIN_VALUE * 16).floatValue() == Float.MIN_VALUE * 16);
		assert (BigRational.valueOf(-Float.MIN_VALUE * 16).floatValue() == -Float.MIN_VALUE * 16);

		// overflow
		assert (BigRational.valueOf(Float.MAX_VALUE).multiply(2).floatValue() == Float.POSITIVE_INFINITY);
		assert (BigRational.valueOf(Float.MAX_VALUE).multiply(4).floatValue() == Float.POSITIVE_INFINITY);
		assert (BigRational.valueOf(Float.MAX_VALUE).multiply(valueOf(1.2f)).floatValue() == Float.POSITIVE_INFINITY);
		assert (BigRational.valueOf(Float.MAX_VALUE).multiply(16).floatValue() == Float.POSITIVE_INFINITY);
		assert (BigRational.valueOf(-Float.MAX_VALUE).multiply(2).floatValue() == Float.NEGATIVE_INFINITY);
		assert (BigRational.valueOf(-Float.MAX_VALUE).multiply(4).floatValue() == Float.NEGATIVE_INFINITY);
		assert (BigRational.valueOf(-Float.MAX_VALUE).multiply(valueOf(1.2f)).floatValue() == Float.NEGATIVE_INFINITY);
		assert (BigRational.valueOf(-Float.MAX_VALUE).multiply(16).floatValue() == Float.NEGATIVE_INFINITY);

		// underflow
		// note that the (float)x==(float)y test yields true for 0.0f==-0.0f
		assert (BigRational.valueOf(Float.MIN_VALUE).divide(2).floatValue() == 0.0f);
		assert (BigRational.valueOf(Float.MIN_VALUE).divide(4).floatValue() == 0.0f);
		assert (BigRational.valueOf(Float.MIN_VALUE).divide(valueOf(1.2f)).floatValue() == 0.0f);
		assert (BigRational.valueOf(Float.MIN_VALUE).divide(16).floatValue() == 0.0f);
		...
		// signed underflow, alternative tests
		assert (String.valueOf(BigRational.valueOf(Float.MIN_VALUE).divide(2).floatValue()).equals("0.0"));
		assert (String.valueOf(BigRational.valueOf(Float.MIN_VALUE).divide(4).floatValue()).equals("0.0"));
		assert (String.valueOf(BigRational.valueOf(Float.MIN_VALUE).divide(valueOf(1.2f)).floatValue()).equals("0.0"));
		assert (String.valueOf(BigRational.valueOf(Float.MIN_VALUE).divide(16).floatValue()).equals("0.0"));
		assert (String.valueOf(BigRational.valueOf(-Float.MIN_VALUE).divide(2).floatValue()).equals("-0.0"));
		assert (String.valueOf(BigRational.valueOf(-Float.MIN_VALUE).divide(4).floatValue()).equals("-0.0"));
		assert (String.valueOf(BigRational.valueOf(-Float.MIN_VALUE).divide(valueOf(1.2f)).floatValue()).equals("-0.0"));
		assert (String.valueOf(BigRational.valueOf(-Float.MIN_VALUE).divide(16).floatValue()).equals("-0.0"));
		...
		// ulp
		assert (BigRational.valueOf(1.0f - StrictMath.ulp(1.0f)).floatValue() == 1.0f - StrictMath.ulp(1.0f));
		assert (BigRational.valueOf(1.0f + StrictMath.ulp(1.0f)).floatValue() == 1.0f + StrictMath.ulp(1.0f));
		assert (BigRational.valueOf(1.0f - StrictMath.ulp(1.0f) / 2).floatValue() == 1.0f - StrictMath.ulp(1.0f) / 2);
		assert (BigRational.valueOf(1.0f + StrictMath.ulp(1.0f) / 2).floatValue() == 1.0f + StrictMath.ulp(1.0f) / 2);
		assert (BigRational.valueOf(1.0f - StrictMath.ulp(1.0f) / 4).floatValue() == 1.0f - StrictMath.ulp(1.0f) / 4);
		assert (BigRational.valueOf(1.0f + StrictMath.ulp(1.0f) / 4).floatValue() == 1.0f + StrictMath.ulp(1.0f) / 4);

		// mantissa rounding
		// a delta of ulp/4 is expected to be rounded
		assert (BigRational.valueOf(1.0f).subtract(BigRational.valueOf(StrictMath.ulp(1.0f)).divide(4)).floatValue() == 1.0f);
		assert (BigRational.valueOf(16.0f).subtract(BigRational.valueOf(StrictMath.ulp(16.0f)).divide(4)).floatValue() == 16.0f);
		// note: MAX_VALUE is not a power-of-two, so it won't run into the mantissa rounding case in question;
		// and 1/MIN_VALUE is larger than MAX_VALUE (due to MIN_VALUE being subnormal)
		// assert (BigRational.valueOf(0x1P127f).subtract(BigRational.valueOf(StrictMath.ulp(0x1P127f)).divide(4)).floatValue() == 0x1P127f);

		// more values in between [0 and max/min]
		assert (BigRational.valueOf((float)StrictMath.sqrt(Float.MAX_VALUE)).floatValue() == (float)StrictMath.sqrt(Float.MAX_VALUE));
		assert (BigRational.valueOf((float)StrictMath.pow(Float.MAX_VALUE, 0.2f)).floatValue() == (float)StrictMath.pow(Float.MAX_VALUE, 0.2f));
		assert (BigRational.valueOf(Float.MAX_VALUE).power(2).floatValue() == Float.POSITIVE_INFINITY);
		assert (BigRational.valueOf(Float.MAX_VALUE).power(5).floatValue() == Float.POSITIVE_INFINITY);
		assert (BigRational.valueOf((float)StrictMath.sqrt(Float.MIN_VALUE)).floatValue() == (float)StrictMath.sqrt(Float.MIN_VALUE));
		assert (BigRational.valueOf((float)StrictMath.pow(Float.MIN_VALUE, 0.2f)).floatValue() == (float)StrictMath.pow(Float.MIN_VALUE, 0.2f));
		assert (BigRational.valueOf(Float.MIN_VALUE).power(2).floatValue() == 0.0f);
		assert (BigRational.valueOf(Float.MIN_VALUE).power(5).floatValue() == 0.0f);
		...
		assert (BigRational.valueOf((float)StrictMath.E).floatValue() == (float)StrictMath.E);
		assert (BigRational.valueOf((float)StrictMath.PI).floatValue() == (float)StrictMath.PI);
		assert (BigRational.valueOf((float)StrictMath.pow((float)StrictMath.E, 2)).floatValue() == (float)StrictMath.pow((float)StrictMath.E, 2));
		assert (BigRational.valueOf((float)StrictMath.sqrt((float)StrictMath.E)).floatValue() == (float)StrictMath.sqrt((float)StrictMath.E));
		assert (BigRational.valueOf((float)StrictMath.pow((float)StrictMath.PI, 2)).floatValue() == (float)StrictMath.pow((float)StrictMath.PI, 2));
		assert (BigRational.valueOf((float)StrictMath.sqrt((float)StrictMath.PI)).floatValue() == (float)StrictMath.sqrt((float)StrictMath.PI));
		...
		// assert (BigRational.valueOf(0.0).floatValue() == (float)0.0);
		// assert (BigRational.valueOf(1.0).floatValue() == (float)1.0);
		// assert (BigRational.valueOf(0.5).floatValue() == (float)0.5);
		// assert (BigRational.valueOf(0.1).floatValue() == (float)0.1);
		// assert (BigRational.valueOf((double)0.1f).floatValue() == (float)(double)0.1f);
		// assert (BigRational.valueOf((double)0.2f).floatValue() == (float)(double)0.2f);
		// assert (BigRational.valueOf((double)0.3f).floatValue() == (float)(double)0.3f);
		// assert (BigRational.valueOf((double)0.4f).floatValue() == (float)(double)0.4f);
		// assert (BigRational.valueOf(1.0 + 0.4 * StrictMath.ulp(1.0f)).floatValue() == (float)(1.0 + 0.4 * StrictMath.ulp(1.0f)));
		// assert (BigRational.valueOf(1.0 + 0.8 * StrictMath.ulp(1.0f)).floatValue() == (float)(1.0 + 0.8 * StrictMath.ulp(1.0f)));
		// assert (BigRational.valueOf(1.0 + 1.6 * StrictMath.ulp(1.0f)).floatValue() == (float)(1.0 + 1.6 * StrictMath.ulp(1.0f)));

		// IEEE 754 constants
		// commented out since not public
		// assert (BigRational.HALF_FLOAT_FRACTION_SIZE + BigRational.HALF_FLOAT_EXPONENT_SIZE + 1 == 0x10);
		// assert (BigRational.SINGLE_FLOAT_FRACTION_SIZE + BigRational.SINGLE_FLOAT_EXPONENT_SIZE + 1 == 0x20);
		// assert (BigRational.DOUBLE_FLOAT_FRACTION_SIZE + BigRational.DOUBLE_FLOAT_EXPONENT_SIZE + 1 == 0x40);
		// assert (BigRational.QUAD_FLOAT_FRACTION_SIZE + BigRational.QUAD_FLOAT_EXPONENT_SIZE + 1 == 0x80);
		// assert (BigRational.HALF_FLOAT_FRACTION_SIZE >= 0);
		// assert (BigRational.HALF_FLOAT_EXPONENT_SIZE >= 0);
		// // assert (BigRational.HALF_FLOAT_EXPONENT_SIZE <= 64 - 1);
		// assert (BigRational.HALF_FLOAT_EXPONENT_SIZE <= 31);
		// assert (BigRational.SINGLE_FLOAT_FRACTION_SIZE >= 0);
		// assert (BigRational.SINGLE_FLOAT_EXPONENT_SIZE >= 0);
		// // assert (BigRational.SINGLE_FLOAT_EXPONENT_SIZE <= 64 - 1);
		// assert (BigRational.SINGLE_FLOAT_EXPONENT_SIZE <= 31);
		// assert (BigRational.DOUBLE_FLOAT_FRACTION_SIZE >= 0);
		// assert (BigRational.DOUBLE_FLOAT_EXPONENT_SIZE >= 0);
		// // assert (BigRational.DOUBLE_FLOAT_EXPONENT_SIZE <= 64 - 1);
		// assert (BigRational.DOUBLE_FLOAT_EXPONENT_SIZE <= 31);
		// assert (BigRational.QUAD_FLOAT_FRACTION_SIZE >= 0);
		// assert (BigRational.QUAD_FLOAT_EXPONENT_SIZE >= 0);
		// // assert (BigRational.QUAD_FLOAT_EXPONENT_SIZE <= 64 - 1);
		// assert (BigRational.QUAD_FLOAT_EXPONENT_SIZE <= 31);

		// direct fromIeee754/toIeee754 methods

		// half (5 exponent bits)
		assert (BigRational.valueOfHalfBits((short)0).equals(BigRational.valueOf("0")));
		assert (BigRational.valueOf("0").halfBitsValue() == 0);
		assert (BigRational.valueOfHalfBits((short)0x3c00).equals(BigRational.valueOf("1")));
		assert (BigRational.valueOf("1").halfBitsValue() == (short)0x3c00);
		assert (BigRational.valueOfHalfBits((short)0xbc00).equals(BigRational.valueOf("-1")));
		assert (BigRational.valueOf("-1").halfBitsValue() == (short)0xbc00);
		assert (BigRational.valueOfHalfBits((short)0x3e00).equals(BigRational.valueOf("1.5")));
		assert (BigRational.valueOf("1.5").halfBitsValue() == (short)0x3e00);
		assert (BigRational.valueOfHalfBits((short)0xbe00).equals(BigRational.valueOf("-1.5")));
		assert (BigRational.valueOf("-1.5").halfBitsValue() == (short)0xbe00);
		assert (BigRational.valueOfHalfBits((short)0x3f00).equals(BigRational.valueOf("1.75")));
		assert (BigRational.valueOf("1.75").halfBitsValue() == (short)0x3f00);
		assert (BigRational.valueOfHalfBits((short)0xbf00).equals(BigRational.valueOf("-1.75")));
		assert (BigRational.valueOf("-1.75").halfBitsValue() == (short)0xbf00);

		// single (8 exponent bits)
		assert (BigRational.valueOfSingleBits(0).equals(BigRational.valueOf("0")));
		assert (BigRational.valueOf("0").singleBitsValue() == 0);
		assert (BigRational.valueOfSingleBits(0x3f800000).equals(BigRational.valueOf("1")));
		assert (BigRational.valueOf("1").singleBitsValue() == 0x3f800000);
		assert (BigRational.valueOfSingleBits(0xbf800000).equals(BigRational.valueOf("-1")));
		assert (BigRational.valueOf("-1").singleBitsValue() == 0xbf800000);
		assert (BigRational.valueOfSingleBits(0x3fc00000).equals(BigRational.valueOf("1.5")));
		assert (BigRational.valueOf("1.5").singleBitsValue() == 0x3fc00000);
		assert (BigRational.valueOfSingleBits(0xbfc00000).equals(BigRational.valueOf("-1.5")));
		assert (BigRational.valueOf("-1.5").singleBitsValue() == 0xbfc00000);
		assert (BigRational.valueOfSingleBits(0x3fe00000).equals(BigRational.valueOf("1.75")));
		assert (BigRational.valueOf("1.75").singleBitsValue() == 0x3fe00000);
		assert (BigRational.valueOfSingleBits(0xbfe00000).equals(BigRational.valueOf("-1.75")));
		assert (BigRational.valueOf("-1.75").singleBitsValue() == 0xbfe00000);

		// double (11 exponent bits)
		assert (BigRational.valueOfDoubleBits(0).equals(BigRational.valueOf("0")));
		assert (BigRational.valueOf("0").doubleBitsValue() == 0);
		assert (BigRational.valueOfDoubleBits(0x3ff0000000000000l).equals(BigRational.valueOf("1")));
		assert (BigRational.valueOf("1").doubleBitsValue() == 0x3ff0000000000000l);
		assert (BigRational.valueOfDoubleBits(0xbff0000000000000l).equals(BigRational.valueOf("-1")));
		assert (BigRational.valueOf("-1").doubleBitsValue() == 0xbff0000000000000l);
		assert (BigRational.valueOfDoubleBits(0x3ff8000000000000l).equals(BigRational.valueOf("1.5")));
		assert (BigRational.valueOf("1.5").doubleBitsValue() == 0x3ff8000000000000l);
		assert (BigRational.valueOfDoubleBits(0xbff8000000000000l).equals(BigRational.valueOf("-1.5")));
		assert (BigRational.valueOf("-1.5").doubleBitsValue() == 0xbff8000000000000l);
		assert (BigRational.valueOfDoubleBits(0x3ffc000000000000l).equals(BigRational.valueOf("1.75")));
		assert (BigRational.valueOf("1.75").doubleBitsValue() == 0x3ffc000000000000l);
		assert (BigRational.valueOfDoubleBits(0xbffc000000000000l).equals(BigRational.valueOf("-1.75")));
		assert (BigRational.valueOf("-1.75").doubleBitsValue() == 0xbffc000000000000l);

		// quadBitsEqual
		assert (BigRational.quadBitsEqual(new long[] {0xfffeffffffffffffl, 0xffffffffffffffffl,}, new long[] {0xfffeffffffffffffl, 0xffffffffffffffffl,}));
		assert (!BigRational.quadBitsEqual(new long[] {0xfffeffffffffffffl, 0xffffffffffffffffl,}, new long[] {0xffffffffffffffffl, 0xfffeffffffffffffl,}));

		// quad (15 exponent bits)
		assert (BigRational.quadBitsEqual(BigRational.valueOf("0").quadBitsValue(), new long[] {0, 0,}));
		assert (BigRational.valueOfQuadBits(new long[] {0x3fff000000000000l, 0,}).equals(BigRational.valueOf("1")));
		assert (BigRational.quadBitsEqual(BigRational.valueOf("1").quadBitsValue(), new long[] {0x3fff000000000000l, 0,}));
		assert (BigRational.valueOfQuadBits(new long[] {0xbfff000000000000l, 0,}).equals(BigRational.valueOf("-1")));
		assert (BigRational.quadBitsEqual(BigRational.valueOf("-1").quadBitsValue(), new long[] {0xbfff000000000000l, 0,}));
		assert (BigRational.valueOfQuadBits(new long[] {0x3fff800000000000l, 0,}).equals(BigRational.valueOf("1.5")));
		assert (BigRational.quadBitsEqual(BigRational.valueOf("1.5").quadBitsValue(), new long[] {0x3fff800000000000l, 0,}));
		assert (BigRational.valueOfQuadBits(new long[] {0xbfff800000000000l, 0,}).equals(BigRational.valueOf("-1.5")));
		assert (BigRational.quadBitsEqual(BigRational.valueOf("-1.5").quadBitsValue(), new long[] {0xbfff800000000000l, 0,}));
		assert (BigRational.valueOfQuadBits(new long[] {0x3fffc00000000000l, 0,}).equals(BigRational.valueOf("1.75")));
		assert (BigRational.quadBitsEqual(BigRational.valueOf("1.75").quadBitsValue(), new long[] {0x3fffc00000000000l, 0,}));
		assert (BigRational.valueOfQuadBits(new long[] {0xbfffc00000000000l, 0,}).equals(BigRational.valueOf("-1.75")));
		assert (BigRational.quadBitsEqual(BigRational.valueOf("-1.75").quadBitsValue(), new long[] {0xbfffc00000000000l, 0,}));

		// more quad tests
		assert (BigRational.quadBitsEqual(BigRational.valueOfQuadBits(new long[] {0x0000000000000000l, 0x0000000000000000l,}).quadBitsValue(), new long[] {0x0000000000000000l, 0x0000000000000000l,}));
		assert (BigRational.quadBitsEqual(BigRational.valueOfQuadBits(new long[] {0x0000000000000001l, 0x0000000000000000l,}).quadBitsValue(), new long[] {0x0000000000000001l, 0x0000000000000000l,}));
		assert (BigRational.quadBitsEqual(BigRational.valueOfQuadBits(new long[] {0x0000000000000000l, 0x8000000000000000l,}).quadBitsValue(), new long[] {0x0000000000000000l, 0x8000000000000000l,}));
		assert (BigRational.quadBitsEqual(BigRational.valueOfQuadBits(new long[] {0x0000000000000000l, 0x0000000000000001l,}).quadBitsValue(), new long[] {0x0000000000000000l, 0x0000000000000001l,}));
		assert (BigRational.quadBitsEqual(BigRational.valueOfQuadBits(new long[] {0x0000000000000002l, 0x0000000000000000l,}).quadBitsValue(), new long[] {0x0000000000000002l, 0x0000000000000000l,}));
		assert (BigRational.quadBitsEqual(BigRational.valueOfQuadBits(new long[] {0x0000000000000000l, 0x4000000000000000l,}).quadBitsValue(), new long[] {0x0000000000000000l, 0x4000000000000000l,}));
		assert (BigRational.quadBitsEqual(BigRational.valueOfQuadBits(new long[] {0x0000000000000000l, 0x0000000000000002l,}).quadBitsValue(), new long[] {0x0000000000000000l, 0x0000000000000002l,}));
		assert (BigRational.quadBitsEqual(BigRational.valueOfQuadBits(new long[] {0x7ffe000000000000l, 0x0000000000000000l,}).quadBitsValue(), new long[] {0x7ffe000000000000l, 0x0000000000000000l,}));
		assert (BigRational.quadBitsEqual(BigRational.valueOfQuadBits(new long[] {0x7ffe000000000001l, 0x0000000000000000l,}).quadBitsValue(), new long[] {0x7ffe000000000001l, 0x0000000000000000l,}));
		assert (BigRational.quadBitsEqual(BigRational.valueOfQuadBits(new long[] {0x7ffe000000000000l, 0x8000000000000000l,}).quadBitsValue(), new long[] {0x7ffe000000000000l, 0x8000000000000000l,}));
		assert (BigRational.quadBitsEqual(BigRational.valueOfQuadBits(new long[] {0x7ffe000000000000l, 0x0000000000000001l,}).quadBitsValue(), new long[] {0x7ffe000000000000l, 0x0000000000000001l,}));
		assert (BigRational.quadBitsEqual(BigRational.valueOfQuadBits(new long[] {0x7ffe000000000002l, 0x0000000000000000l,}).quadBitsValue(), new long[] {0x7ffe000000000002l, 0x0000000000000000l,}));
		assert (BigRational.quadBitsEqual(BigRational.valueOfQuadBits(new long[] {0x7ffe000000000000l, 0x4000000000000000l,}).quadBitsValue(), new long[] {0x7ffe000000000000l, 0x4000000000000000l,}));
		assert (BigRational.quadBitsEqual(BigRational.valueOfQuadBits(new long[] {0x7ffe000000000000l, 0x0000000000000002l,}).quadBitsValue(), new long[] {0x7ffe000000000000l, 0x0000000000000002l,}));

		// forth and back

		// most bits possible set (i.e. largest negative)
		assert (BigRational.valueOfHalfBits((short)0xfbff).halfBitsValue() == (short)0xfbff);
		assert (BigRational.valueOfSingleBits(0xff7fffff).singleBitsValue() == 0xff7fffff);
		assert (BigRational.valueOfDoubleBits(0xffefffffffffffffl).doubleBitsValue() == 0xffefffffffffffffl);
		assert (BigRational.quadBitsEqual(BigRational.valueOfQuadBits(new long[] {0xfffeffffffffffffl, 0xffffffffffffffffl,}).quadBitsValue(), new long[] {0xfffeffffffffffffl, 0xffffffffffffffffl,}));

		// smallest non-subnormal number
		assert (BigRational.valueOfHalfBits((short)0x0400).halfBitsValue() == (short)0x0400);
		assert (BigRational.valueOfSingleBits(0x00800000).singleBitsValue() == 0x00800000);
		assert (BigRational.valueOfDoubleBits(0x0010000000000000l).doubleBitsValue() == 0x0010000000000000l);
		assert (BigRational.quadBitsEqual(BigRational.valueOfQuadBits(new long[] {0x0001000000000000l, 0x0000000000000000l,}).quadBitsValue(), new long[] {0x0001000000000000l, 0x0000000000000000l,}));
		// largest subnormal number
		assert (BigRational.valueOfHalfBits((short)0x02ff).halfBitsValue() == (short)0x02ff);
		assert (BigRational.valueOfSingleBits(0x004fffff).singleBitsValue() == 0x004fffff);
		assert (BigRational.valueOfDoubleBits(0x0008ffffffffffffl).doubleBitsValue() == 0x0008ffffffffffffl);
		assert (BigRational.quadBitsEqual(BigRational.valueOfQuadBits(new long[] {0x00008fffffffffffl, 0xffffffffffffffffl,}).quadBitsValue(), new long[] {0x00008fffffffffffl, 0xffffffffffffffffl,}));
		// largest subnormal number with one bit set only
		assert (BigRational.valueOfHalfBits((short)0x0200).halfBitsValue() == (short)0x0200);
		assert (BigRational.valueOfSingleBits(0x00400000).singleBitsValue() == 0x00400000);
		assert (BigRational.valueOfDoubleBits(0x0008000000000000l).doubleBitsValue() == 0x0008000000000000l);
		assert (BigRational.quadBitsEqual(BigRational.valueOfQuadBits(new long[] {0x0000800000000000l, 0x0000000000000000l,}).quadBitsValue(), new long[] {0x0000800000000000l, 0x0000000000000000l,}));
		// one less of above
		assert (BigRational.valueOfHalfBits((short)0x01ff).halfBitsValue() == (short)0x01ff);
		assert (BigRational.valueOfSingleBits(0x003fffff).singleBitsValue() == 0x003fffff);
		assert (BigRational.valueOfDoubleBits(0x0007ffffffffffffl).doubleBitsValue() == 0x0007ffffffffffffl);
		assert (BigRational.quadBitsEqual(BigRational.valueOfQuadBits(new long[] {0x00007fffffffffffl, 0xffffffffffffffffl,}).quadBitsValue(), new long[] {0x00007fffffffffffl, 0xffffffffffffffffl,}));
		// half of above
		assert (BigRational.valueOfHalfBits((short)0x0100).halfBitsValue() == (short)0x0100);
		assert (BigRational.valueOfSingleBits(0x00200000).singleBitsValue() == 0x00200000);
		assert (BigRational.valueOfDoubleBits(0x0004000000000000l).doubleBitsValue() == 0x0004000000000000l);
		assert (BigRational.quadBitsEqual(BigRational.valueOfQuadBits(new long[] {0x0000400000000000l, 0x0000000000000000l,}).quadBitsValue(), new long[] {0x0000400000000000l, 0x0000000000000000l,}));

		// round-off vs. exact
		assert (BigRational.valueOfHalfBits(BigRational.valueOf("0.125").halfBitsValue()).equals(BigRational.valueOf("0.125")));
		assert (BigRational.valueOfSingleBits(BigRational.valueOf("0.125").singleBitsValue()).equals(BigRational.valueOf("0.125")));
		assert (BigRational.valueOfDoubleBits(BigRational.valueOf("0.125").doubleBitsValue()).equals(BigRational.valueOf("0.125")));
		assert (BigRational.valueOfQuadBits(BigRational.valueOf("0.125").quadBitsValue()).equals(BigRational.valueOf("0.125")));
		assert (!BigRational.valueOfHalfBits(BigRational.valueOf("0.1").halfBitsValue()).equals(BigRational.valueOf("0.1")));
		assert (!BigRational.valueOfSingleBits(BigRational.valueOf("0.1").singleBitsValue()).equals(BigRational.valueOf("0.1")));
		assert (!BigRational.valueOfDoubleBits(BigRational.valueOf("0.1").doubleBitsValue()).equals(BigRational.valueOf("0.1")));
		assert (!BigRational.valueOfQuadBits(BigRational.valueOf("0.1").quadBitsValue()).equals(BigRational.valueOf("0.1")));
